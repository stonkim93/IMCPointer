...
* **안정성 개선**: `Process` 핸들 누수 버그 수정으로 장기 실행 안정성 확보
* **정확도 향상**: `IMEPali` 프로세스 감지 로직 개선으로 영어 사용자의 오작동(Pali 오인식) 문제 해결 (5초 캐싱 적용)
* **성능 최적화**: 윈도우 메시지(`WndProc`) 디바운싱(debouncing) 적용으로 불필요한 시스템 리소스 낭비 차단
* **사용자 경험 향상**: 트레이 메뉴 설정(포인터 모드, 작은 원 표시 등)이 레지스트리에 저장되어 재실행 시에도 유지됨
